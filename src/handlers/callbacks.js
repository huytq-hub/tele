...
async function handleDepositCheck(bot, query) {
  ...
  if (result.confirmed) {
    ...
    let msg = `✅ ${t('deposit_success', { amount: formatPrice(result.amount, displayCurrency) })}\n💰 +${formatPrice(creditedAmount)} USDT`;
    ...
    config.ADMIN_IDS.forEach(id => bot.sendMessage(id, `💰 DEPOSIT\n👤 ${result.user_id}\n💵 ${formatPrice(result.amount, displayCurrency)} → ${formatPrice(creditedAmount)} USDT\n📱 ${result.payment_method}`));
  } else {
